...
struct RingView: View {
    ...
    private func ratio(for categoryIndex: Int) -> Double {
       ...
       let cumExp =
        transactionVM.cummulativeExpense(for:Category.allCases[categoryIndex])
    
        let result = cumExp / transactionVM.totalForAllButUnpinnedTransactions()
    ...
    private func offset(for categoryIndex: Int) -> Double {
       ...
       // Double(categoryIndex) * 0.2
        ...
        if categoryIndex == 0 {
            ...
            for pos in 0..<categoryIndex {
                totalRatio += ratio(for: pos)
            }
        }
        return totalRatio
    ...
    private func percentageText(for categoryIndex: Int) -> String {
        let computedPercent = ratio(for: categoryIndex) * 100
        if computedPercent > 0 {
            return "\(computedPercent.formatted(hasDecimals: false))%"
        }
        return ""
    }
...
struct RingView_Previews: PreviewProvider {
    static var sampleRing: some View {
        ZStack {
            ...
            RingView.PartialCircleShape(offset:0.65 /*0.65*/, ratio: 0.35)
                .stroke(
                    Color.blue,
                    style: StrokeStyle(lineWidth: 28.0, lineCap: .butt)
                )
